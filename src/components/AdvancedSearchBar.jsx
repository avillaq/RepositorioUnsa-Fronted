...
const options = ["Logica", "Lenguaje natural"];

export const AdvancedSearchBar = ({ onSearch }) => {
  const [query, setQuery] = useState('');
  const [selected, setSelected] = useState(options[0]);
  const [placeholder, setPlaceholder] = useState("Ejemplo: Pedagogia and Educacion");
  ...
  const handleSelected = (option) => {
    setSelected(option);
    if (option === "Logica") {
      setPlaceholder("Ej. Pedagogia and Educacion");
    } else {
      setPlaceholder("Ej. ¿Cuales son los beneficios de la educacion a distancia?");
    }
  };
    ...
    <form className="max-w-lg mx-auto" onSubmit={handleSearch}>
      <div className="flex">
          ...
          <ul className="py-2 text-sm text-gray-700" aria-labelledby={dropdownButtonId}>
            {
              options.map((option, index) => (
                <li key={index}>
                  <button type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100" onClick={() => handleSelected(option)}>{option}</button>
                </li>
              ))
            }
          </ul>
        ...
        <div className="relative w-full">
          <input type="search" id={searchInputId} className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-e-lg border-s-gray-50 border-s-2 border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder={placeholder} required value={query} onChange={handleInputChange} />
